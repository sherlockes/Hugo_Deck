...
*   **📸 Inserción de Imágenes Inteligente (Insert Image)**: Un botón integrado en el pie del editor lateral te permite incorporar capturas de pantalla, fotos o la portada (Thumbnail) al instante. Soporta la subida de archivos `*.jpg` locales o pegar directamente una captura desde el portapapeles (`Ctrl+V`), la cual es automáticamente redimensionada a un ancho optimizado de 550px manteniendo su proporción. Las imágenes se guardan con nombres secuenciales inteligentes en la carpeta `repo/static/images/` siguiendo la regla de nomenclatura `{fecha}_{slug_del_articulo}_{num}.jpg`, e inserta de forma inmediata la referencia `![image-{num}]` en el cursor junto con su declaración de ruta al final del archivo. También permite subir el **Thumbnail** de portada, el cual se guarda automáticamente con el sufijo fijo `_00.jpg`, sobreescribiéndose si ya existe.
*   **🗑️ Eliminación de Artículos (Delete)**: Botón integrado en el pie del editor lateral que elimina permanentemente el archivo `.md` del artículo actual del repositorio local, previa confirmación explícita del usuario. Tras la eliminación el editor se cierra y el iframe navega automáticamente a la raíz del sitio.
*   **📦 Mover Artículos (Move)**: Botón integrado en el pie del editor lateral que abre un modal con un árbol de directorios expandible y navegable, limitado a la carpeta `content/` del repositorio. Selecciona la carpeta de destino con un clic, confirma la ruta final en el campo de texto inferior y pulsa "Mover aquí" para reubicar el archivo manteniendo su nombre original.
*   **🔗 Inserción de Enlaces Relativos (Enlace)**: Botón integrado en el pie del editor lateral que abre un modal con el listado completo y buscable de todos los artículos del repositorio. Al seleccionar uno, inserta automáticamente un enlace Markdown `[Título del artículo](/url/)` en la posición exacta del cursor. Si hay texto previamente seleccionado en el editor, este se usa como texto del enlace en lugar del título. El cambio dispara el auto-guardado al instante.
...
*   **Asistente de Inserción de Imágenes:** En la parte inferior del editor lateral contarás con el botón "Insertar Imagen" (que reemplaza al botón "Cancelar" para optimizar el espacio). Al hacer clic en él se abrirá un panel integrado sobre tu área de trabajo que te dará tres opciones: buscar un archivo de imagen, activar el modo portapapeles o subir el **Thumbnail** de portada. Al pegar o seleccionar el archivo para artículos, el sistema se encarga de optimizar, renombrar de forma correlativa e insertar la sintaxis `![image-{num}]` de Markdown en la posición exacta de tu cursor, añadiendo su ruta de referencia al final del documento de forma automatizada. En el caso de seleccionar el **Thumbnail**, se guardará con el nombre `{fecha}_{slug}_00.jpg` sobreescribiendo el existente si corresponde.
